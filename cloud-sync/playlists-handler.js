...
export function handleCloudPlaylistsOptions() {
    return new Response(null, { status: 204, headers: JSON_HEADERS });
}

export async function handleCloudPlaylistsGet(request, env) {
...
export async function handleCloudPlaylistsPut(request, env) {
    const auth = await authenticate(request, env);
    if (auth.response) return auth.response;
